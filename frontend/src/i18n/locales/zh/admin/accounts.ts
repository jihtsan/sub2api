export default {
    accounts: {
      traffic: {
        "title": "可选流量控制与运行观测",
        "description": "默认关闭。严格 RPM 按账号共享预算，并发建议仅供观察，不自动调整固定并发。初始参数不是上游账号的实际额度。",
        "grok": "Grok 实时语音仅支持按会话观察；开启严格 RPM 后会拒绝实时语音连接，HTTP 语音仍可使用。",
        "strict": "启用严格 RPM",
        "rpm": "每分钟硬上限",
        "burst": "瞬时突发额度",
        "budgetHint": "滚动 60 秒内不超过硬上限；突发额度包含在上限内。HTTP、测试和重试共享预算，OpenAI WS 按 response.create 轮次计数。",
        "observe": "启用并发观察建议",
        "min_concurrency": "建议并发下限",
        "failure_threshold": "触发失败次数",
        "failure_window_seconds": "失败统计窗口（秒）",
        "recovery_seconds": "稳定恢复周期（秒）",
        "observeHint": "429 / 5xx 达到阈值后建议减半；稳定期内至少成功 3 次后逐步恢复。固定并发 {limit} 保持不变。",
        "saveHint": "与账号其他设置一起保存；关闭开关即可停用。",
        "loading": "正在刷新…",
        "refresh": "刷新已保存配置的状态",
        "unavailable": "观测暂不可用。严格 RPM 无法确认预算时暂停新请求；仅观察模式不因观测缓存故障阻断请求。",
        "inactive": "已保存配置未启用流量观测。",
        "duration": "平均响应耗时",
        "statsHint": "最近一分钟为滚动窗口；累计数在缓存过期或重置后重新统计。",
        "effective_concurrency": "有效并发上限",
        "recommended_concurrency": "建议并发",
        "in_flight": "执行中",
        "requests_last_minute": "最近一分钟请求",
        "upstream_429": "上游 429",
        "upstream_5xx": "上游 5xx",
        "rejected_rpm": "本地 RPM 拒绝"
},
      title: '账号管理',
