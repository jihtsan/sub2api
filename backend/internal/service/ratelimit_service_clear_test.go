...
func TestRateLimitService_RecoverAccountAfterSuccessfulTest_PreservesUnrelatedRestrictions(t *testing.T) {
	now := time.Now()
	repo := &rateLimitClearRepoStub{
		getByIDAccount: &Account{
			ID:                     42,
...
func TestRateLimitService_RecoverAccountAfterSuccessfulTest_DoesNotInvokeErrorReset(t *testing.T) {
	repo := &rateLimitClearRepoStub{
		getByIDAccount: &Account{
			ID:     9,
			Status: StatusError,
