...
func TestAccountTrafficPolicyRejectsInvalidValues(t *testing.T) {
	for _, input := range []map[string]any{{"rpm": 0}, {"rpm": 1, "burst": 2}, {"adaptive_mode": "unknown"}, {"adaptive_mode": "automatic"}, {"strict_rpm_enabled": "true"}, {"unexpected": 1}} {
...
func TestAccountTrafficLeaseLossOnlyCancelsEnforcedRequests(t *testing.T) {
	for _, mode := range []string{"observe", "strict"} {
		t.Run(mode, func(t *testing.T) {
			cache := &trafficCacheStub{acquired: true}
			policy := DefaultAccountTrafficPolicy()
			policy.AdaptiveEnabled = true
			policy.StrictRPMEnabled = mode == "strict"
			...
			require.Equal(t, mode == "strict", closed)
			require.Equal(t, mode == "strict", ctx.Err() != nil)
			require.Equal(t, 1, cache.finishes)
			permit.Finish(200)
...
func TestAccountTrafficWSOverlappingLanesDoNotImposeExtraConcurrency(t *testing.T) {
	cache := &trafficCacheStub{acquired: true}
	inner := &trafficFrameStub{}
	policy := DefaultAccountTrafficPolicy()
	policy.AdaptiveEnabled = true
	frame := &accountTrafficFrameConn{inner: inner, control: NewAccountTrafficService(cache), plan: AccountTrafficPlan{AccountID: 42, Policy: policy}, ctx: context.Background()}
	for _, lane := range []string{"planner", "research", "planner"} {
		require.NoError(t, frame.WriteFrame(context.Background(), coderws.MessageText, []byte(`{"type":"response.create","stream_id":"`+lane+`"}`)))
	}
	require.Equal(t, 3, cache.calls)
	require.Equal(t, 3, inner.writes)
	inner.payload = []byte(`{"type":"response.completed","stream_id":"research"}`)
	_, _, err := frame.ReadFrame(context.Background())
	require.NoError(t, err)
	require.Equal(t, 1, cache.finishes)
	require.Len(t, frame.pending["planner"], 2)
	require.NoError(t, frame.Close())
	require.Equal(t, 3, cache.finishes)
}

type trafficAccountReader struct {
	AccountRepository
	account *Account
	err     error
}

func (r trafficAccountReader) GetByID(context.Context, int64) (*Account, error) {
	return r.account, r.err
}

func TestAccountTrafficExistingWSSessionReadsNewPolicyEachTurn(t *testing.T) {
	cache := &trafficCacheStub{acquired: true}
	account := &Account{ID: 42, Concurrency: 8}
	old, err := AccountTrafficPlanFor(account)
	require.NoError(t, err)
	control := ProvideAccountTrafficService(cache, trafficAccountReader{account: account})
	_, permit, err := control.Begin(context.Background(), old)
	require.NoError(t, err)
	require.Nil(t, permit)
	account.Extra = map[string]any{AccountTrafficPolicyKey: map[string]any{"strict_rpm_enabled": true}}
	cache.acquired = false
	_, permit, err = control.Begin(context.Background(), old)
...
func TestAccountTrafficIncompletePooledWSTurnCannotRecover(t *testing.T) {
	cache := &trafficCacheStub{acquired: true}
	policy := DefaultAccountTrafficPolicy()
	policy.AdaptiveEnabled = true
	_, permit, err := NewAccountTrafficService(cache).Begin(context.Background(), AccountTrafficPlan{AccountID: 42, Policy: policy})
	require.NoError(t, err)
	permit.observeEvent([]byte(`{"type":"response.incomplete"}`))
	finishAccountTrafficTurn(permit, nil)
	require.Equal(t, 499, cache.status)
}

func TestAccountTrafficLateLeaseRefreshCannotCloseCompletedWSConnection(t *testing.T) {
	cache := &trafficCacheStub{acquired: true}
	policy := DefaultAccountTrafficPolicy()
	policy.StrictRPMEnabled = true
	_, permit, err := NewAccountTrafficService(cache).Begin(context.Background(), AccountTrafficPlan{AccountID: 42, Policy: policy})
	...
	closed := false
	permit.loseLease([]func(){func() { closed = true }})
	require.False(t, closed, "an in-flight refresh finishing after release cannot close the socket reused by a new turn")
	require.Equal(t, 1, cache.finishes)
	require.Equal(t, 200, cache.status)
}
