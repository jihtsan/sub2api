...
type AccountTrafficService struct {
	cache    AccountTrafficCache
	accounts AccountRepository
}

// The repository refresh ensures long-lived WS sessions use the current policy
// on each turn. Ordinary accounts without a traffic policy retain the HTTP fast path.
func ProvideAccountTrafficService(cache AccountTrafficCache, accounts AccountRepository) *AccountTrafficService {
	return &AccountTrafficService{cache: cache, accounts: accounts}
}
...
func AccountTrafficFailover(err error) *UpstreamFailoverError {
	...
	if errors.As(err, &failover) && failover.Reason == "account_traffic_limit" {
		return failover
	}
	return nil
}
...
func finishAccountTrafficTurn(permit *AccountTrafficPermit, err error) {
	...
	status := 0
	if terminal := permit.terminalStatus.Load(); terminal != 0 {
		status = int(terminal)
	} else if err == nil {
		status = 200
	} else {
...
func (s *AccountTrafficService) Begin(ctx context.Context, plan AccountTrafficPlan, onLeaseLost ...func()) (context.Context, *AccountTrafficPermit, error) {
	...
	if ctx.Value(accountTrafficCoveredKey{}) == plan.AccountID {
		return ctx, nil, nil
	}
	if s != nil && s.accounts != nil && plan.AccountID > 0 {
		account, err := s.accounts.GetByID(ctx, plan.AccountID)
		if err != nil {
			if !plan.Policy.Enforces() {
				return ctx, nil, nil
			}
			return ctx, nil, &AccountTrafficLimitError{Reason: "账号流量配置暂不可用，请稍后重试", Status: 503, RetryAfter: time.Second}
		}
		plan, err = AccountTrafficPlanFor(account)
		if err != nil {
			return ctx, nil, &AccountTrafficLimitError{Reason: "账号流量配置无效", Status: 400}
		}
	}
...
func (p *AccountTrafficPermit) loseLease(callbacks []func()) {
	if p.plan.Policy.Enforces() {
		p.finishObservation(0, true, callbacks...)
		return
	}
...
func (p *AccountTrafficPermit) finishObservation(status int, cancelRequest bool, callbacks ...func()) {
	p.once.Do(func() {
		close(p.stop)
...
func (s *AccountTrafficService) DoHTTP(req *http.Request, send func(*http.Request) (*http.Response, error)) (*http.Response, error) {
	...
	if !ok || req.Context().Value(accountTrafficCoveredKey{}) == plan.AccountID {
		return send(req)
	}
