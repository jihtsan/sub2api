...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if err != nil {
		if local := AccountTrafficFailover(err); local != nil {
			c.Header("Retry-After", local.ResponseHeaders.Get("Retry-After"))
			s.countTokensError(c, local.StatusCode, AccountTrafficErrorType(local.StatusCode), local.ClientMessage)
			return local
		}
