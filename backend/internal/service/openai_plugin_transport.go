...
func (s *OpenAIGatewayService) doOpenAIUpstream(request *http.Request, proxyURL string, account *Account) (*http.Response, error) {
	...
	return accountTrafficController(s.httpUpstream).DoHTTP(request, func(request *http.Request) (*http.Response, error) {
		return s.doOpenAIUpstreamWithoutTraffic(request, proxyURL, account)
	})
}
...
func (s *AccountTestService) doOpenAIAccountTestUpstream(
	...
	request = WithAccountTrafficRequest(request, account)
	return accountTrafficController(s.httpUpstream).DoHTTP(request, func(request *http.Request) (*http.Response, error) {
		return s.doOpenAIAccountTestUpstreamWithoutTraffic(request, proxyURL, account, useTLSFallback)
	})
}
