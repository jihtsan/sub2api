...
// token-bucket burst allowance and in-flight observations. Every key for
...
local minimum=math.min(tonumber(ARGV[9]),math.max(1,hard))
...
if rev<oldrev and oldsig~=sig then
 if not rpmOn and redis.call('HGET',KEYS[1],'enforces')=='0' then
...
redis.call('HSET',KEYS[1],'enforces',rpmOn and '1' or '0')
